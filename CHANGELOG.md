...
All notable changes to impact-ledger are documented here.
...
- **`__version__` is now derived from installed package metadata** via
  `importlib.metadata.version("impact-ledger")` instead of a hardcoded string.
...
- **`setup.py` reduced to a build shim** — the stale `version="0.2.0"` pin and
